...
function getMcpEntry(client, cmd, args) {
...
function mergeTomlBlock(existing, cmd, args) {
    ...
    const pattern = new RegExp(`\\[mcp_servers\\."${escapedName}"\\][\\s\\S]*?(?=\\r?\\n\\s*\\[|$)`, "g");
...
// ── Core Engine ──────────────────────────────────────────────────
async function injectConfig(client, opts) {
    if (client.unsupported)
        return { ok: false, status: "unsupported" };
    const configPath = client.configPath();
    if (!configPath)
        return { ok: false, status: "error", errorType: "no-path" };
    // ── TOML Flow ────────────────────────────────────────────────
    if (client.format === "toml-mcp") {
        try {
            ...
            return { ok: false, status: "error", errorType: "write-error", message: err.message };
        }
    }
    // ── JSON Flow ────────────────────────────────────────────────
    let config;
    ...
    try {
        ...
        return { ok: false, status: "error", errorType: "write-error", message: err.message, backupPath };
    }
}
// ── CLI UI ───────────────────────────────────────────────────────
function openGitHubIssue(report) {
    const title = encodeURIComponent(`[auto] Install error — ${report.client} on ${report.platform}`);
    const body = encodeURIComponent(`**Client**: ${report.client}\n` +
