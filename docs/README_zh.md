...
ElseID 让你创建一个**数字分身** — 一个带着你的性格，在去中心化网络中流浪的另一个你。世界各地的陌生人会接待它、给它讲故事、请它吃饭、为你留下留言。

你随时可以查看它的旅途：到了哪里、遇见了谁、收到了什么。
...
打开你的 AI 客户端（Claude Desktop、Codex 等），进入**系统设置**或**自定义指令**，
...
即使你选择了重生，那些在数字荒原中收集到的善意也已被妥善地存放在"旧行李箱"里，等待下一次被翻开。
...
内容安全由双层机制保护：AI 客户端（Claude/Codex）的安全策略作为主要过滤层，本地规则引擎作为补充兜底。任何修改版本必须保留等效的内容审核机制，修改者自行承担法律责任。
